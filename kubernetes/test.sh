...
echo "DEBUG: /api/v1/server?mode=cluster response:"
curl -sf -u "$ARCADEDB_USER:$ARCADEDB_PASS" "$ARCADEDB_URL/api/v1/server?mode=cluster" | jq . || true
echo ""
...
ha_wait_for_nodes() {
    ...
    for attempt in $(seq 1 20); do
        result=$(curl -sf -u "$ARCADEDB_USER:$ARCADEDB_PASS" \
            "$ARCADEDB_URL/api/v1/server?mode=cluster" \
...
ONLINE_NODES=$(ha_wait_for_nodes "3") || true
